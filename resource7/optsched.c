...
int main(int argc , char *argv[])
{
	...
	//for(i=1; i<=CLIENT_MAX+1; i++)
	{
		...
		{
				if(arr[0][0]*j+iNewIntervalStartTime <= (TIME_MAX+iNewIntervalStartTime))
				{
	...
	//printf("arr[iMinLCDIntervalIdx][1]=%d, iNewIntervalStartTime=%d\n", arr[iMinLCDIntervalIdx][1], iNewIntervalStartTime);
	//for(i=1; i<=CLIENT_MAX+1; i++)
	{
		if(arr[iMinLCDIntervalIdx][1] > iNewIntervalStartTime)
		{
			//iTempGap = arr[iMinLCDIntervalIdx][i] - iNewIntervalStartTime;
			iTempGap = arr[iMinLCDIntervalIdx][1];
			//break;
		}
		else
		{
			iTempGap = iNewIntervalStartTime;
			//iTempGap = 0;
		}
	}
	...
	if(iTempGap >= arr[0][0])
	{
		...
		for(j=0; j<=SLOT_MAX+1; j++)
		{
				if(arr[0][0]*j+iTempGap <= (TIME_MAX+iNewIntervalStartTime))
				{
	...
	for(i=0; i<=(arr[0][0]/QUANTUM); i++)
	{
		...
		for(j=0; j<=SLOT_MAX+1; j++)
		{
				if((arr[0][0]*j+abc) <= (TIME_MAX+iNewIntervalStartTime))
				{
					arr[0][j+1] = arr[0][0]*j+abc;
					//printf("(arr[0][0]*j+iNewIntervalStartTime=%d, (TIME_MAX+iNewIntervalStartTime)=%d\n", 
					//	(arr[0][0]*j+iNewIntervalStartTime),
					//	(TIME_MAX+iNewIntervalStartTime));
					addTempQueue(arr[0][j+1]);
				}
				else
				{
